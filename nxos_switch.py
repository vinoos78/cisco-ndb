...
class NXOSSwitch:
    ...
    def __init__(self, ip_address, con_type='ssh', username='admin', password='nbv12345', ndb_controller=None, device_params=dict()):

        self.ip_address = ip_address
        self.con_type = con_type
        self.username = username
        self.password = password
        self.ndb_controller = ndb_controller
    ...
    def register_openflow_device(self):
        ...
        print("Pipeline = %d \n Controller IP = %s \n Controller port = 6653 " % (self.pipeline, self.ndb_controller.ip_address))
        config = "\n"
        ...
        ''' % (self.pipeline, self.ndb_controller.ip_address, config)
        swHdl = self.handle
        swHdl.sendline(cmd)
        swHdl.sendline('\r')
        swHdl.expect('# $')
